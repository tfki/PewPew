...
pub fn load_all_textures<'a>(resources: &mut Resources<'a>, texture_creator: &'a TextureCreator<WindowContext>) -> Result<HashMap<String, usize>, LoadAllTexturesError> {
    ...
    for (idx, path) in files_to_load.iter().enumerate() {
        //                           -------------------------wat?--------------------------
        if file_idx_map.insert(path.file_name().unwrap().to_str().unwrap().to_string(), idx).is_some() {
            return Err(LoadAllTexturesError::DuplicateName);
        }

        resources.images.push(texture_creator.load_texture(path).map_err(|_| LoadAllTexturesError::FileNotFound)?);
    }
