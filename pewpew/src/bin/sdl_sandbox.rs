...
pub fn print_display_information(video: &sdl2::VideoSubsystem) {
    println!("\n");
    println!("default is_screen_saver_enabled: {}", video.is_screen_saver_enabled().to_string());
...
pub fn display_intro(canvas: &mut sdl2::render::WindowCanvas) {
    ...
    let huhn_in_hole_spritesheet = texture_creator.load_texture(Path::new("ressources/Huhn_in_Hole.png")).unwrap(); // https://onlinetools.com/image/remove-specific-color-from-image
    ...
    dest_rect_hih.center_on(Point::new((SCREEN_WIDTH as i32) / 2, (SCREEN_HEIGHT as i32) / 2)); // wo fängt das Rect an aufm Bildschirm
...
pub fn display_flying_huhns(canvas: &mut sdl2::render::WindowCanvas) {
    ...
    let texture_creator = canvas.texture_creator();
    let huhns_fyling_dying_spritesheet = texture_creator.load_texture(Path::new("ressources/Huhns_flying_dying.png")).unwrap(); // https://onlinetools.com/image/remove-specific-color-from-image
    ...
    let mut dest_rect_medium = Rect::new(0, 0, sprite_tile_size_medium.0 * 4, sprite_tile_size_medium.1 * 4);
    ...
    dest_rect_close.center_on(Point::new(CLOSE_WIDTH_SPRITE as i32 * -4, ((SCREEN_HEIGHT as f64) * 0.6).round() as i32));
    dest_rect_medium.center_on(Point::new(MEDIUM_WIDTH_SPRITE as i32 * -4, ((SCREEN_HEIGHT as f64) * 0.4).round() as i32));
    dest_rect_far.center_on(Point::new(FAR_WIDTH_SPRITE as i32 * -4, ((SCREEN_HEIGHT as f64) * 0.2).round() as i32));
    ...
    for flying_anim_counter in 0..FLYING_NUM_ANIMS * NUM_OF_REPS {
        ...
        dest_rect_close.set_x(((flying_anim_counter * CLOSE_WIDTH_SPRITE) / TRAVEL_SPEED_DIVIDER) as i32);
        dest_rect_medium.set_x(((flying_anim_counter * MEDIUM_WIDTH_SPRITE) / TRAVEL_SPEED_DIVIDER) as i32);
        dest_rect_far.set_x(((flying_anim_counter * FAR_WIDTH_SPRITE) / TRAVEL_SPEED_DIVIDER) as i32);
